<?php
    ...
    $defaultalarms = [
        ...
        ['web_name' => 'pump10', 'description' => 'Ошибка насосов Н4/5', 'status' => 1],
        ...
        ['web_name' => 'hwR22', 'description' => 'Аварийно высокий уровень воды в резервуаре Р22', 'status' => 1],
        ...
        ['web_name' => 'almR24', 'description' => 'Ошибка датчика уровня резервуара R24', 'status' => 1],
        ...
        ['web_name' => 'krohne', 'description' => 'Ошибка связи с расходомером Optimass', 'status' => 0]
    ];
    try{
        ...
        if ($mysqli->connect_errno) {
            ...
            if($res){
                ...
                if($res){
                    ...
                    $sql = "";
                    $firstdate = date('Y-m-d H:i:s', strtotime('-'.(count($defaultalarms)*10).' hours'));
                    for($i = 0; $i < count($defaultalarms); $i++){
                        $dateon = $firstdate;
                        $dateoff = ($defaultalarms[$i]["status"] == 0) ? "'".date('Y-m-d H:i:s', strtotime($dateon.'+'.intval(mt_rand(2,15)).' minutes'))."'" : 'null';
                        $sql = "INSERT INTO `".$tblname."` (`dateon`,`dateoff`,`status`,`web_name`,`description`) VALUES 
                        ('".$dateon."',".$dateoff.",'".$defaultalarms[$i]["status"]."','".$defaultalarms[$i]["web_name"]."','".$defaultalarms[$i]["description"]."')";
                        $res = $mysqli->query($sql);
                        if($res){
                        ...
                        $firstdate = date('Y-m-d H:i:s', strtotime($dateon.'+'.intval(mt_rand(20,40)).' minutes'));
                    }
                    if(!$res){
                        $mysqli->close();
